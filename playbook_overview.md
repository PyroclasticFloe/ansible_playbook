...
| `install_quadlets.yml` | Copies `services/<name>/quadlets/*.container` to `/etc/containers/systemd/<name>/`, then templates the `<name>.pod` file from `service.pod.j2` (bridged network + Tailscale MagicDNS). Deletes any deployed `*.container`/`*.pod` not in the expected set so renamed quadlets don't linger. Never touches `<etc_dir>` secrets. Notifies `Restart service` (and `Reload systemd` when pruning) on change. |
