...
- Karakeep 502 follow-up: `HOSTNAME=0.0.0.0` never reached the Next.js process (its `/proc` env still showed `HOSTNAME=karakeep`, so it kept binding the UTS hostname), and `kara-chrome.service` never generated (`Init` is not a valid Quadlet `Container` key). Pointed `serve.json` at `http://karakeep:3000` (resolves pod-wide via pod `/etc/hosts`, verified from both containers) instead of `127.0.0.1:3000`; replaced `Init=true` with `PodmanArgs=--init` and collapsed the multi-line `Exec` to a single line in `kara-chrome.container`. **Done**
